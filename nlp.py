...
def lemma_text(text):
    lemmatizedtext=[(wnl.lemmatize(word),word) for word in text]
...
def stemm(mydata):
	...
	if request.method =='POST':
		return redirect(url_for('lemma',mydata=mydata))
	return render_template('stemm.html', stemm=data_stems)

@app.route('/lemma/<mydata>',methods=['GET', 'POST'])
def lemma(mydata):
	data_token=tokenize(mydata)
	#dt_nostpwrds=rmvstpwrds(data_token)
	data_lemma=lemma_text(data_token)
	if request.method =='POST':
		return redirect(url_for('summary',mydata=mydata))
	return render_template('lemma.html', lemma=data_lemma)
